...
/**
 * 请事假流程审批完成监听器
 * 流程标识Key：hr_personal_leave
 * 审批通过后自动将请假区间内的考勤状态更新为事假（status=7）
 */
@Component
@Slf4j
public class HrPersonalLeaveStatusListener extends BpmProcessInstanceStatusEventListener
    ...
    protected void onEvent(BpmProcessInstanceStatusEvent event) {
        ...
        try {
            ...
            if (leaveStartDate == null || leaveEndDate == null) {
                log.warn("[HrPersonalLeaveStatusListener] 流程实例 {} 缺少请假日期变量", processInstanceId);
                return;
            }

            // status=7=事假，leaveType=2
            getAttendanceService().updateLeaveStatus(userId, leaveStartDate, leaveEndDate, 7, 2, processInstanceId);
            log.info("[HrPersonalLeaveStatusListener] userId={} 事假 {}-{} 考勤已更新", userId, leaveStartDate, leaveEndDate);
        } catch (Exception e) {
            log.error("[HrPersonalLeaveStatusListener] 处理流程 {} 失败", processInstanceId, e);
        }
    }
